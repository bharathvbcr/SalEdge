...
export const StockTakeModal: React.FC<StockTakeModalProps> = ({ onClose }) => {
    ...
    const [counts, setCounts] = useState<Record<string, string>>(() =>
        Object.fromEntries(stockInventory.map(i => [i.id, i.stock.toString()]))
    );
    ...
    const currencySymbol = config.firms[0]?.financials.currencySymbol || '₹';
    ...
    }, [stockInventory, productTypes, counts, search]);
    ...
    return (
        ...
        <Modal onClose={onClose} size="xl" ariaLabel="Stock Take">
            <ModalHeader
                ...
                subtitle="Enter physical counts for all stock"
                onClose={onClose}
            />
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {rows.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="p-8 text-center text-text-muted">No inventory batches found.</td>
                                    </tr>
